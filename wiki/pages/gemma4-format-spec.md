...
1. **zipcode sends plain OpenAI `/v1/chat/completions` JSON** (`crates/inference/src/llama_server_backend.rs:293-378`). Roles: `system` / `user` / `assistant` / `tool`. Content is always a flat string (or `null` for tool-call-only assistant turns). Tools are passed as `{type:"function", function:{name, description, parameters}}`.
