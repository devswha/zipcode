...
5. **Path safety:** if the tool reads/writes files, route every path through `resolve_and_validate_path()` (`tools/lib.rs:19`). See [tools › path safety](tools.md#path-safety-resolve_and_validate_path).
...
2. Injections added after the base:
   ...
   - `.zipcode.md` content from the detected project root if present (`crates/runtime/src/prompt.rs:51-63`)
