...
Graphify-style summary of the zipcode `crates/` workspace as of 2026-04-20 (provenance anchors refreshed).
...
| 2 | `ConversationLoop` | `crates/runtime/src/conversation.rs:29` | Holds `Box<dyn InferenceProvider>` + `ToolRegistry` + `PermissionPolicy` + `Session` + `system_prompt`. Every turn flows through `run_turn()`. |
| 3 | `Tool` trait + `ToolRegistry` | `crates/tools/src/lib.rs:367,375` | 10 implementors; `execute_tool()` (`:419`) is the single entry point from `runtime`. |
...
| 5 | `PermissionMode` | `crates/tools/src/lib.rs:282` | Referenced by `ToolContext`, `PermissionPolicy`, CLI args, config. Gates every tool call. |
...
| 7 | `resolve_and_validate_path()` | `crates/tools/src/lib.rs:14` | Called by every file-touching tool (read, write, edit, glob, grep). Single path-safety gate. |
...
2. **The conversation loop's iteration cap lives in `runtime`, but the tool-result truncation cap lives in `tools`.** `MAX_TOOL_ITERATIONS = 25` (`crates/runtime/src/conversation.rs:54`) and `MAX_TOOL_OUTPUT_BYTES = 8192` (`crates/tools/src/lib.rs:416`). Two independent safety limits, two independent crates. **Why it matters:** a runaway agent is bounded by both — model iterations AND per-tool output size.
...
5. **Permission policy is split across two crates.** `PermissionMode` enum lives in `tools` (`:282`); `PermissionPolicy::check()` lives in `runtime` (`permission.rs:28`). **Why it matters:** adding a new permission tier requires editing both crates.

6. **Session persistence is JSON-per-file, not a database.** `~/.zipcode/sessions/{uuid}.json` (`crates/runtime/src/session.rs:38`). **Why it matters:** `ls ~/.zipcode/sessions/ | wc -l` scales linearly forever; no retention policy.
...
1. **"Where does a tool call actually execute?"** → `crates/tools/src/lib.rs:419` (`execute_tool`) called from `crates/runtime/src/conversation.rs:105-143`. Both pages cross-link.
2. **"What stops the agent from looping forever?"** → Two bounds: 25 tool iterations (`conversation.rs:54`) and 8 KB per tool result (`tools/lib.rs:416`).
3. **"Why doesn't model X work?"** → Almost always the hardcoded Gemma chat template. See [`chat-template`](pages/chat-template.md).
4. **"Where is `~/.zipcode/config.json` read?"** → `crates/runtime/src/config.rs:140` (`ZipcodeConfig::load`), merged with `.zipcode.json` from project root.
5. **"Which tools are blocked in read-only mode?"** → Everything except `read_file`, `glob_search`, `grep_search`, `tool_search`. `crates/runtime/src/permission.rs:28-49`.
...
7. **"How is the system prompt built?"** → `crates/runtime/src/prompt.rs:27-59`: base prompt + permission line + cwd + `.zipcode.md` content.
...
| 4 | Tool output silently truncated at 8 KB; model isn't told how much was cut | `tools/lib.rs:416,297-317` |
| 5 | Tool iteration cap of 25 returns an error, not a graceful handoff | `conversation.rs:54` |
...
| 9 | Session files grow unbounded in `~/.zipcode/sessions/` (no rotation) | `session.rs:38` |
| 10 | Env var names for GPU are case-sensitive; typos silently ignored | `config.rs` + CLI env read |

---

## Test surface (269 passing + 2 ignored on current workspace)
...
| `runtime` | 39 | 12 (`tests/integration.rs`) | `MockInferenceProvider` drives loop tests including read-only denial, workspace-write approval accept/reject, traversal blocking, persistence, and compaction resume |
| `cli` | 71 | 43 smoke tests (`tests/smoke.rs`) | Unit tests live across `commands`, `repl`, `render`, `tui`, `tui_composer`, and `width`; smoke tests spawn the real binary in temp `HOME` |
