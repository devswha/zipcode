...
Graphify-style summary of the zipcode `crates/` workspace as of 2026-04-15.
...
| 2 | `ConversationLoop` | `crates/runtime/src/conversation.rs:23` | Holds `Box<dyn InferenceProvider>` + `ToolRegistry` + `PermissionPolicy` + `Session` + `system_prompt`. Every turn flows through `run_turn()`. |
| 3 | `Tool` trait + `ToolRegistry` | `crates/tools/src/lib.rs:328,336` | 10 implementors; `execute_tool()` (`:380`) is the single entry point from `runtime`. |
...
| 5 | `PermissionMode` | `crates/tools/src/lib.rs:262` | Referenced by `ToolContext`, `PermissionPolicy`, CLI args, config. Gates every tool call. |
...
2. **The conversation loop's iteration cap lives in `runtime`, but the tool-result truncation cap lives in `tools`.** `MAX_TOOL_ITERATIONS = 25` (`crates/runtime/src/conversation.rs:43`) and `MAX_TOOL_OUTPUT_BYTES = 8192` (`crates/tools/src/lib.rs:377`). Two independent safety limits, two independent crates. **Why it matters:** a runaway agent is bounded by both — model iterations AND per-tool output size.
...
5. **Permission policy is split across two crates.** `PermissionMode` enum lives in `tools` (`:262`); `PermissionPolicy::check()` lives in `runtime` (`permission.rs:28`). **Why it matters:** adding a new permission tier requires editing both crates.
...
1. **"Where does a tool call actually execute?"** → `crates/tools/src/lib.rs:380` (`execute_tool`) called from `crates/runtime/src/conversation.rs:105-143`. Both pages cross-link.
2. **"What stops the agent from looping forever?"** → Two bounds: 25 tool iterations (`conversation.rs:43`) and 8 KB per tool result (`tools/lib.rs:377`).
...
| 4 | Tool output silently truncated at 8 KB; model isn't told how much was cut | `tools/lib.rs:377,297-317` |
| 5 | Tool iteration cap of 25 returns an error, not a graceful handoff | `conversation.rs:43` |
