...
2. **The conversation loop's iteration cap lives in `runtime`, but the tool-result truncation cap lives in `tools`.** `MAX_TOOL_ITERATIONS = 25` (`crates/runtime/src/conversation.rs:110`) and `MAX_TOOL_OUTPUT_BYTES = 8192` (`crates/tools/src/lib.rs:502`). Two independent safety limits, two independent crates. **Why it matters:** a runaway agent is bounded by both — model iterations AND per-tool output size.
...
6. **Session persistence is JSON-per-file, not a database.** `~/.zipcode/sessions/{uuid}.json` (`crates/runtime/src/session.rs:80`). **Why it matters:** `ls ~/.zipcode/sessions/ | wc -l` scales linearly forever; no retention policy.
