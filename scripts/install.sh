...
write_next_steps() {
...

    {
        ...
        echo "  3. Load the generated environment:"
        echo "     source \"${SETUP_ENV}\""
        echo "  4. Verify the install:"
        echo "     zipcode doctor"
        echo "  5. Run a prompt:"
        echo "     zipcode prompt \"hello\""
        ...
        if [ -n "${llama_server_path}" ]; then
            ...
            echo "If Gemma 4 needs the fallback server later, install it with:"
            echo "  \"${helper_path}\" /path/to/llama-server \"${INSTALL_DIR}\""
        else
            echo ""
            echo "No bundled llama-server was found."
            echo "If Gemma 4 needs the fallback server later, set:"
