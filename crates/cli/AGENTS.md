...
The CLI embeds a build label generated by `crates/cli/build.rs`: package version, short Git commit, tracked dirty marker, and Unix build timestamp. Keep `zipcode --version`, `zipcode doctor`, plain REPL startup, and fullscreen TUI headers aligned when changing version display.
...
- Fullscreen TUI renders on the main terminal screen and mirrors transcript entries to scrollback so mouse scroll/selection can copy logs
